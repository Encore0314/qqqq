...
var Mbox = {
	presets: {
		...
		cancel: '取消',
		clostxt: '×',
		loadimg: 'http://admin.tdche8.com/images/wait1.gif',
	...
	build: function() {
		...
		if (!$("mboxWindow")) {
			//页面上无mbox元素
			var boxConstr = '<span id="mboxBar" class="mbox_bar">'+this.options.title+'</span><a href="javascript:;" id="mboxBtnClose" class="mbox_close" title="关闭">'+this.options.clostxt+'</a><div id="mboxContent" class="mbox_cont"></div><div id="mboxOperate" class="mbox_operate" style="display:none;"></div>';
	...
	close: function() {
		if (this.isOpen) {
			...
			//保护装载元素
			if (this.elementObj) {
				this.elementObj.setStyle("display", "none");
			}
